...
from compression.sender_mbmv2_small import encoding
from compression.reciever_mbmv2_small import decompress_cfg, decoding
...
if __name__ == "__main__":
    ...
    initial_model_path='/workspace/Infinity_codec/local_output/debug_stage2_2026_5_08_mbm_v2_small_3090/slim-checkpoint/slim-ckpt-giter020K-ep0-iter20000-last.pth'
    ...
    args=argparse.Namespace(
        ...
        use_flex_attn=0,
        cache_dir='/workspace/Infinity_codec/local_output/debug_stage2_2026_5_02_mbm_v2',
    ...
    for current_filename in models_to_test:
        ...
        args.rec_path = f'/workspace/Infinity_codec/results/debug_stage2_2026_5_08_mbm_v2_small_3090_step4_441014_20k_bernoulli_sample/DIV2K_dynamic_{current_iter}'
        ...
        for data in json_data:
            ...
            # ======== MBM 修复: 保持所有 Scale 的通道数为 args.vae_type (32位) ========
            # ======== 终极修复 2：使用 MBM 模型的真实比特数 ========
            scale_q = [(scale_schedule[i][0], scale_schedule[i][1], scale_schedule[i][2], 
                        infinity.codebook_dim) for i in range(len(scale_schedule))]
            # =========================================================================
            # Prompt bits
            prompt_bits = 0
            prompt_packet = None
            ...
            # amp_dtype = torch.bfloat16 if getattr(args, 'bf16', 0) == 1 else torch.float32
            # with torch.no_grad(), torch.amp.autocast('cuda', dtype=amp_dtype, enabled=amp_dtype==torch.bfloat16):
    ...
    if getattr(args, 'keep_gpu_busy', 0) == 1:
        ...
        if len(json_data) > 0:
            ...
            # ======== MBM 修复: 强制所有 Scale 对齐 args.vae_type (32位) ========
            dummy_scale_q = [(scale_schedule[i][0], scale_schedule[i][1], scale_schedule[i][2], 
                        args.vae_type) for i in range(len(scale_schedule))]
            # ====================================================================
