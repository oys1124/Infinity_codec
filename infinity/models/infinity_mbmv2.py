...
class SwiGLU(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None):
        ...
        hidden_features = hidden_features or int(in_features * 4 * 2 / 3)
...
# 2. 100% 契合的 EntropyBitHead
class EntropyBitHead(nn.Module):
    def __init__(self, embed_dim, num_bits=32, hidden_dim=None):
        super().__init__()
        self.num_bits = num_bits
        self.hidden_dim = hidden_dim or embed_dim
        
        # 【核心修改】新增映射层。输入维度：Context + 真实的 Bits(32维) + Mask状态(32维)
        self.input_proj = nn.Linear(embed_dim + num_bits * 2, self.hidden_dim)
        
        self.net = nn.Sequential(
            SwiGLU(self.hidden_dim, self.hidden_dim * 2, self.hidden_dim),
            SwiGLU(self.hidden_dim, self.hidden_dim * 2, self.hidden_dim),
            nn.LayerNorm(self.hidden_dim),
            nn.Linear(self.hidden_dim, self.num_bits)
        )

    def forward(self, context, known_bits=None, bit_mask=None):
        B, L, _ = context.shape
        # 如果是 Step-1 或者普通算术编码阶段，全为未知 (0)
        if known_bits is None or bit_mask is None:
            known_bits = torch.zeros((B, L, self.num_bits), device=context.device, dtype=torch.float32)
            bit_mask = torch.zeros((B, L, self.num_bits), device=context.device, dtype=torch.float32) # 0 表示 Masked(未知)
            
        # 拼接！模型现在真正“看见”了已知比特
        combined_input = torch.cat([context, known_bits, bit_mask], dim=-1)
...
class Infinity(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.mbm_head = EntropyBitHead(embed_dim=self.C, num_bits=self.codebook_dim)
    ...
    def get_logits(self, h: torch.Tensor, cond_BD: Optional[torch.Tensor]):
        ...
        """
        """
        修改为：直接返回融合了 cond_BD 的上下文特征 (Context)，不直接过分类头。
        """
    ...
    # ==================== 新增：HIERAMP 获取掩码接口 ====================
    def get_hieramp_mask(self):
        last_ca = None
        # 遍历网络寻找最后一层 CrossAttention
        for chunk in self.block_chunks:
            for m in chunk.module:
                if hasattr(m, 'ca'):  # 如果是 CrossAttnBlock，它会有 .ca 属性
                    last_ca = m.ca
        
        # 防御性检查
        if last_ca is None or not hasattr(last_ca, 'last_spatial_mask'):
            return None
            
        mask = last_ca.last_spatial_mask # 形状: [B, SeqLen]
        
        # 归一化到 0 ~ 1 之间 (Min-Max Scaling)
    ...
    def mbm_training_forward(self, context, gt_bits):
        """
        供外层 Trainer 调用的纯净训练前向过程
        不再注入噪声，只为了获取全量的上下文概率
        """
        # 直接将 context 送入头，获取 32 bits 的 logits
        logits = self.mbm_head(context)
        
        # 返回 logits，第二个返回值给 None (为了兼容 trainer 解包 mbm_logits, mbm_mask)
        return logits, None
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        cfg_infer=False, gt_bits=None,
        # 【新增这两个参数】
        known_bits=None, bit_mask=None,
        **kwargs,
        ...
        # [3. unpad the seqlen dim, and then get logits]
        context = self.get_logits(x_BLC[:, :l_end], cond_BD)
        # 【修改这里】：如果传入了 known_bits 和 bit_mask，说明我们在做真正的 MBM 训练前向！
        if known_bits is not None and bit_mask is not None:
            return self.mbm_head(context, known_bits, bit_mask)
        ...
        **kwargs
        ...
        for item in cfg_insertion_layer:
            if item == 0: 
                ...
                assert leng+item > 0, f'cfg_insertion_layer: {item} is not valid'
        ...
        for si, pn in enumerate(scale_schedule): 
           ...
           # ================= MBM 4 步渐进解码取代原独立采样 =================
            ...
            # 1. 始终获取未混合的 Context (尺寸可能是 B 也可能是 2B)
            # 绝对不能在这里做 CFG 插值！
            if cfg != 1.0:
                context = self.get_logits(last_stage, cond_BD) # [2B, L, C]
            else:
                context = self.get_logits(last_stage[:B], cond_BD[:B]) # [B, L, C]

            # 真正的 B_real 永远是 B
            B_real = B
            num_bits = self.codebook_dim
            L_inf = context.shape[1]
            
            # 状态只维护真实的 B 维度
            final_bits = torch.zeros((B_real, L_inf, num_bits), device=context.device, dtype=torch.float32)
            bit_mask = torch.zeros((B_real, L_inf, num_bits), device=context.device, dtype=torch.float32)

            num_steps = 4
            for step in range(num_steps):
                # 适配 CFG 的 batch 维度。如果是 CFG=2.0，模型同时跑 Cond 和 Uncond
                if cfg != 1.0:
                    mbm_input_bits = final_bits.repeat(2, 1, 1)
                    mbm_input_mask = bit_mask.repeat(2, 1, 1)
                else:
                    mbm_input_bits = final_bits
                    mbm_input_mask = bit_mask

                # MBM Head 前向！
                mbm_logits = self.mbm_head(context, mbm_input_bits, mbm_input_mask)

                # 【正统的 CFG 插值必须在这里做！】
                if cfg != 1.0:
                    cond_logits, uncond_logits = mbm_logits.chunk(2, dim=0)
                    mbm_logits_final = uncond_logits + cfg * (cond_logits - uncond_logits)
                else:
                    mbm_logits_final = mbm_logits

                # 转为概率
                probs = torch.sigmoid(mbm_logits_final / tau_list[si])

                # 计算置信度
                confidence = torch.abs(probs - 0.5)
                # 把已经锁定的比特置信度设为极大负数，防止后续被重复选中
                ...
                if step == num_steps - 1:
                    bits_to_lock_total = num_bits # 最后一步锁定所有剩余的
    ...
    def special_init(
        ...
        # ======== MBM 修改 4: 替换旧的 head 初始化逻辑 ========
        # 兼容我们新加的 mbm_head，对其最后一层 Linear 进行 scale_head 初始化
        if hasattr(self, 'mbm_head'):
            # 注意：这里改成了 self.net[-1]
            self.mbm_head.net[-1].weight.data.mul_(scale_head)
            if hasattr(self.mbm_head.net[-1], 'bias') and self.mbm_head.net[-1].bias is not None:
                self.mbm_head.net[-1].bias.data.zero_()
