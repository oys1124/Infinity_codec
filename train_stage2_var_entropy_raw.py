...
def build_everything_from_args(args: arg_util.Args, saver):
    ...
    from trainer_stage2_var_entropy_raw import InfinityTrainer
    # build trainer
...
def build_model_optimizer(args, vae_ckpt):
    ...
    if args.rush_resume:
        ...
        def drop_unfit_weights(state_dict):
            ...
            if 'head.weight' in state_dict:
                if not hasattr(gpt_wo_ddp, 'head'):
                    del state_dict['head.weight']
                    if 'head.bias' in state_dict:
                        del state_dict['head.bias']
                elif state_dict['head.weight'].shape[0] != gpt_wo_ddp.head.out_features:
                    del state_dict['head.weight']
                    if 'head.bias' in state_dict:
                        del state_dict['head.bias']
            
            # 2. [新增] 清理失效的 mbm_head 权重
            keys_to_delete = []
            for k, v in state_dict.items():
                if k.startswith('mbm_head.'):
                    # 如果新模型没有这个 key，或者 shape 对不上，直接丢弃，让它随机初始化
                    if k not in gpt_wo_ddp.state_dict():
                        keys_to_delete.append(k)
                    elif gpt_wo_ddp.state_dict()[k].shape != v.shape:
                        keys_to_delete.append(k)
                if k.startswith('nisp_head.') or k.startswith('shallow_nitp_head.'):
                    if k not in gpt_wo_ddp.state_dict():
                        keys_to_delete.append(k)
                    elif gpt_wo_ddp.state_dict()[k].shape != v.shape:
                        keys_to_delete.append(k)
    ...
    if args.zero:
        ...
        try:
            ...
            # PyTorch 2.0.1 环境下可能在 experimental 路径，或者根本没有
            try:
                from torch.distributed._device_mesh import init_device_mesh
            except ImportError:
                init_device_mesh = None
                print("[Warning] init_device_mesh not found, skipping multi-dimensional mesh initialization.")
        ...
        if args.enable_hybrid_shard:
            ...
            if init_device_mesh is not None:
                device_mesh = init_device_mesh(args.device, (args.data_parallel_size, args.model_parallel_size))
            else:
                device_mesh = None
        else:
