...
NISP_SCALE_MIN=${NISP_SCALE_MIN:-0}
NISP_SCALE_MAX=${NISP_SCALE_MAX:-12}
NISP_HEAD_TYPE=${NISP_HEAD_TYPE:-mlp}
NISP_HEAD_HIDDEN_DIM=${NISP_HEAD_HIDDEN_DIM:-1024}
MODEL=${MODEL:-2bc8}
if [ -z "${VAE_TYPE:-}" ]; then
...
exp_name=${EXP_NAME:-stage2_1024_${MODEL}_${VAE_TYPE}vae_dino_nisp_${NISP_MODE}_scale${NISP_SCALE_MIN}-${NISP_SCALE_MAX}_${NISP_HEAD_TYPE}${NISP_HEAD_HIDDEN_DIM}_w${NISP_WEIGHT}}
...
if [ "${VAE_TYPE}" = "16" ]; then
  ...
  VAE_CKPT=${VAE_CKPT:-/workspace/Infinity_codec/outputs/bitvae_tok_stage1_dino0.1_2_pretrain/checkpoints/model_step_249999.ckpt}
